...
export default function Home() {
  ...
  const handleLater = () => {
    ...
    localStorage.setItem("republic_day_later", nextTime.toISOString());
      ...
      {reviewOrder && (
        <ReviewPopup
          ...
          onClose={() => {
            ...
            localStorage.setItem(`review_later_${reviewOrder.id}`, nextTime);
            setReviewOrder(null);
          }}
          onSuccess={() => {
            localStorage.setItem(`review_done_${reviewOrder.id}`, "1");
            setReviewOrder(null);
          }}
        />
      )}
      {showRepublicPopup && <RepublicDayPopup onLater={handleLater} />}
    </>
  );
}
